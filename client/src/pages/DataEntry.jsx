...
import { Database, Send, Briefcase, User, Loader2, Calendar, Building2, CreditCard, Hash } from 'lucide-react';
...
const DataEntry = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // CRITICAL FIX: Sanitizing payload using getNum to prevent sending "" (empty string) strings to numeric SQL rows
    const payload = {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/salary/submit', {
        ...
        headers: {
          ...
          ...(token && { 'Authorization': `Bearer ${token}` }) // Attaches JWT token safely if present
        },
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm outline-none transition-all uppercase"
                />
              </div>
            </div>
